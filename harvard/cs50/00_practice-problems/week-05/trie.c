...
// Return true if item is found in the trie, false if not found
bool check(char *word)
{
    // To navigate the trie
    node *cursor = root;

    for (int i = 0, n = strlen(word); i < n; i++)
    {
        // To make upper and lowercase equivalent
        int idx = tolower(word[i]) - 'a';

        if (cursor->children[idx] == NULL)
        {
            // Hit NULL before word has been fully traversed so word cannot exist
            return false;
        }
        else
        {
            // The next letter exists in the trie so traverse to it
