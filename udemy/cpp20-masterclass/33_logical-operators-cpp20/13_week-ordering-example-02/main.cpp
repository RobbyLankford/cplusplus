...
class CaseInsensitiveString
{
    public:
        ...
        bool operator==(const CaseInsensitiveString &right_op) const
        {
            return (case_insensitive_compare(s.c_str(), right_op.s.c_str()) == std::weak_ordering::equivalent) ? true : false;
        }
...
int main()
{
    ...
    // Have to create a == operator (as done); compiler will not generate one
    std::cout << "ci_str1 == ci_str2: " << (ci_str1 == ci_str2) << std::endl;
